...
# LOG File save
log_file = os.path.join(MAIN_FOLDER, 'logs/extraction.log')
...
def get_all_emails_in_folder(account, folder:(exchangelib.folders.known_folders.Inbox or exchangelib.folders.known_folders.Messages), after=None)->pd.DataFrame:
    ...
    now = EWSDateTime.now()
    logger.debug(f'MAIL BOX FOLDER NAME: {folder}')
...
def save_email_df(path, df:pd.DataFrame, email_box_name:str, date:str=None)->None:
    if date:
        file_path = os.path.join(path, f'{email_box_name}_{date}.csv')
    else:
        file_path = os.path.join(path, f'{email_box_name}.csv')
    df.to_csv(file_path, index=False)
    logger.debug(f'File save in: {file_path}')

def main():
    for mail_box in MAIL_BOXES[:]:
        mail_box_name = mail_box.split('@')[0] + '_' + mail_box.split('@')[1].split('.')[0]
        logger.info(f'MAIL BOX NAME: {mail_box_name}')
        ...
        if not df.empty:
            logger.debug(f'DF NOT EMPTY: no rows {df.shape[0]}')
            db.df_to_sql(df)


if __name__ == '__main__':
    print('Process start')
    logger.info('NEW EXCUTION')
    try:
        global_credentials = Credentials(EMAIL, PASSWORD)
        main()
        clean_backup(backup_folder)
    except Exception as e:
        logging.error('PROCESS ERROR', exc_info=True)

    print('Process end')
    logger.info('END EXCUTION')